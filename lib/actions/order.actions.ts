...
export async function approvePayPalOrder(orderId: string, data: { orderID: string }) {
	try {
		...
		if (!capturedData || capturedData.id !== (order.paymentResult as PaymentResult)?.id || capturedData.status !== 'COMPLETED') {
	...
	sendPurchaseReceipt({
		order: {
			...
			orderitems: updatedOrder.orderitems.map((item: any) => ({
				...item,
				// If size is an array, take the first element; otherwise, keep as is
